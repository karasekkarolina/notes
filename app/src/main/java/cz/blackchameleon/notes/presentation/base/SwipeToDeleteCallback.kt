...
import cz.blackchameleon.notes.*

/**
 * SwipeToDeleteCallback
 * Handles whole logic for redrawing item in notes list.
 * The actions starts when user swipes on item to left.
 *
...
abstract class SwipeToDeleteCallback(context: Context) : ItemTouchHelper.SimpleCallback(0, ItemTouchHelper.LEFT) {
  ...
  override fun onChildDraw(canvas: Canvas, recyclerView: RecyclerView, viewHolder: RecyclerView.ViewHolder, dX: Float, dY: Float, actionState: Int, isCurrentlyActive: Boolean) {
    ...
    // If user stops the movement which indicates the deletion.
    if (isCanceled) {
    ...
    // Draws red background of separate item.
    background.color = backgroundColor
    background.setBounds(itemView.right + dX.toInt(), itemView.top, itemView.right, itemView.bottom)
    background.draw(canvas)

    // Calculates position of thrash bin icon.
